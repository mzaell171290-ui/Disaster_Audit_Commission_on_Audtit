// ============================================================
// SITE SEARCH
// Searches across Reports, Articles, and the Documentation
// Library using only plain JS. Matches are shown in a dropdown
// under the hero search box; clicking a result opens the right
// modal/panel and scrolls it into view.
// ============================================================
var siteSearchIndex = [
...
// ============================================================
// ARTICLE SLIDE-IN PANEL LOGIC
// Opens a <dialog> as a top layer (showModal) and then animates
// it sliding in from the right edge using the inline transform
// that's already set on each .article-panel dialog. Closing
// reverses the animation before actually closing the dialog.
// ============================================================
function openArticle(id) {
  var dlg = document.getElementById(id);
  if (!dlg) return;
  dlg.showModal();
  // Force the slide-in on the next frame so the transition fires
...
// Clicking the dimmed backdrop area of an article panel also closes it,
// and resets the panel off-screen so it's ready to slide in next time.
...
// Swaps known English phrases for their Filipino translations
// across all text nodes in the document body.
// ============================================================
(function () {
  ...
  const phrases = {
    ...
    "Full Photo Gallery": "Buong Gallery ng Larawan",
...
// ============================================================
// VIDEO CAROUSEL (map section)
// Plain JS, no transitions/animations — just swaps which video
// is loaded into the <video> element and updates the dots.
//
// Edit videoCarouselVideos below to add/remove/reorder videos,
// or to change the poster (preview image) shown before each
// video plays. "poster" can be left as "" if you don't want one.
// ============================================================
...
function videoCarouselRender() {
  ...
  video.load(); // reload the <video> element so the new source takes effect
  video.pause();
