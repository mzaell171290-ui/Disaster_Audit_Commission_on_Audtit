/* =====================================================================
   PRISM — vanilla JS port of the React <Prism /> component.
   Same WebGL shader, same options, no React/JSX involved.
   Requires the "ogl" library — imported below straight from a CDN,
   so there is no build step and no npm install needed.
   ===================================================================== */
import { Renderer, Triangle, Program, Mesh } from 'https://esm.sh/ogl@1.0.6';
...
export function createPrism(container, options = {}) {
  ...
  // returns a cleanup function — call it if you ever remove the section
  return function destroyPrism() {
    stopRAF();
    ro.disconnect();
    if (animationType === 'hover') {
